...
```{r, message=FALSE, warning=FALSE}
setwd("C:/Users/33677/Documents/NEW_WORK/THESE/Permian-Triassic_assembly process/Dataset ammonoids")
paleocoord <- read.csv2("ammo_paleocoord.csv", dec=",", header = TRUE, row.names = 1)
...
data.changhsingien <- read.csv("ammo_Ch.csv", sep = ";", header = TRUE, row.names=1)
data.grieshbachien <- read.csv("ammo_Gr.csv", sep = ";", header = TRUE, row.names=1)
data.dienerien <- read.csv("ammo_Di.csv", sep = ";", header = TRUE, row.names=1)
data.smithien <- read.csv("ammo_Sm.csv", sep = ";", header = TRUE, row.names=1)
data.spathien <- read.csv("ammo_Sp.csv", sep = ";", header = TRUE, row.names=1)
...
l <- list( #grouper les tableaux d'occurrences en une liste pour tout faire en même temps
  ...
  Griesbachian = list(Gr=as.data.frame(t(data.grieshbachien)), name = "Griesbachian"),
...
Sm <- list.data.nos$Smithian$Sm[!rownames(list.data.nos$Smithian$Sm)%in%c("Ala","Jap","Cau","Afg","Vie","Pri","Mon"),]
...
Sp <-  list.data.nos$Spathian$Sp[!rownames(list.data.nos$Spathian$Sp)%in%c("Sib","Jap","Vie","Qin","Pri","Yug"),]
...
sapply(names(anosimper), function (x) write.csv2(as.data.frame(anosimper[[x]][[1]]), 
                                                 file=paste0("SIMPER_data_", x, "_ammonoid.csv"),
                                                 row.names = FALSE))
...
ggplot(data=DNCI.df.1, mapping = aes(x=DNCI, y=factor(id, levels=c("Changhsingian","Griesbachian","Dienerian","Smithian","Spathian"))))+
  ...
  labs(x="DNCI", y="")
DNCI.plot.1
ggsave(file="DNCI_conodont.svg", plot=DNCI.plot.1, 
