---
title: "Macroecology of Early Triassic ammonoids"
author: "Neo Nicollet & Pauline Guenser"
date: "`r Sys.Date()`"
...
```{r, message=FALSE, warning=FALSE}
paleocoord <- read.csv("https://raw.githubusercontent.com/PaulineGnsr/Permian-Triassic_assembly-process/refs/heads/main/Dataset%20ammonoids/ammo_paleocoord.csv", 
                        dec=",", sep=";", header = TRUE, row.names = 1)
paleocoord.cent <- paleocoord[,!names(paleocoord)=="Region_Neo"]
paleocoord.cent$OffPaleolong[which(paleocoord.cent$OffPaleolong<25)] <- paleocoord.cent$OffPaleolong[which(paleocoord.cent$OffPaleolong<25)] + 360
paleocoord.cent <- paleocoord.cent[order(paleocoord.cent$OffPaleolong),]

data.changhsingien <- read.csv("https://raw.githubusercontent.com/PaulineGnsr/Permian-Triassic_assembly-process/refs/heads/main/Dataset%20ammonoids/ammo_Ch.csv", 
                               sep = ";", header = TRUE, row.names=1)
data.griesbachien <- read.csv("https://raw.githubusercontent.com/PaulineGnsr/Permian-Triassic_assembly-process/refs/heads/main/Dataset%20ammonoids/ammo_Gr.csv", 
                               sep = ";", header = TRUE, row.names=1)
data.dienerien <- read.csv("https://raw.githubusercontent.com/PaulineGnsr/Permian-Triassic_assembly-process/refs/heads/main/Dataset%20ammonoids/ammo_Di.csv", 
                           sep = ";", header = TRUE, row.names=1)
data.smithien <- read.csv("https://raw.githubusercontent.com/PaulineGnsr/Permian-Triassic_assembly-process/refs/heads/main/Dataset%20ammonoids/ammo_Sm.csv", 
                          sep = ";", header = TRUE, row.names=1)
data.spathien <- read.csv("https://raw.githubusercontent.com/PaulineGnsr/Permian-Triassic_assembly-process/refs/heads/main/Dataset%20ammonoids/ammo_Sp.csv", 
                          sep = ";", header = TRUE, row.names=1)

l <- list( #grouper les tableaux d'occurrences en une liste pour tout faire en même temps
  Changhsingian = list(Ch=as.data.frame(t(data.changhsingien)), name = "Changhsingian"),
  Griesbachian = list(Gr=as.data.frame(t(data.griesbachien)), name = "Griesbachian"),
  Dienerian = list(Di=as.data.frame(t(data.dienerien)), name = "Dienerian"),
  Smithian = list(Sm=as.data.frame(t(data.smithien)), name = "Smithian"),
  Spathian = list(Sp=as.data.frame(t(data.spathien)), name = "Spathian")
)

list.data <- lapply(l, function(data){
  rname <- rownames(data[[1]])
  data[[1]][data[[1]]=="1?"] <- 1
  data[[1]] <- as.data.frame(sapply(data[[1]], as.numeric))
  rownames(data[[1]]) <- rname
  return(data)
})
...
##Get plate models on the computer (here Scotese)
pathToPolygons <- file.path("C:/Users/33677/Documents/Etudes/THESE/Base de donnée/DATABASE/Tuto paléo GPS/Scotese PaleoAtlas_v3/PALEOMAP Global Plate Model","PALEOMAP_PlatePolygons.gpml")
pathToRotations <- file.path("C:/Users/33677/Documents/Etudes/THESE/Base de donnée/DATABASE/Tuto paléo GPS/Scotese PaleoAtlas_v3/PALEOMAP Global Plate Model","PALEOMAP_PlateModel.rot")
pm <- rgplates::platemodel(polygons=pathToPolygons, rotation=pathToRotations)
# data.paleocoord.off.PALEOMAP <- rgplates::reconstruct(paleocoord[,!names(paleocoord)=="Region_Neo"], age=250, model=pm, plateperiod=FALSE)

#Plot paleocoordinates
mapOff250 <- rgplates::reconstruct("coastlines", age=250, model="PALEOMAP")#("plates", age=250, model=pm)
par(mai=c(0,0,0,0))
svg(filename="paleomap_sections_raw_ammonoid.svg", width = 8)
plot(mapOff250$geometry, border=NA, col="gray")
points(paleocoord[,!names(paleocoord)=="Region_Neo"], cex=0.75, pch=16, col="darkgoldenrod")
text(paleocoord[,!names(paleocoord)=="Region_Neo"], 
     labels=paleocoord$Region_Neo, cex=0.5)
dev.off()
```
...
### IV.a) PCoA and UPGMA classification
...
#Transformer les tableaux d'incidence en matrices de dissimilarité (une pour chaque intervalle de temps)
...
### IV.e) Assembly process: DNCI calculation
