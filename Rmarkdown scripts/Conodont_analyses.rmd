...
This is an R Markdown document which synthesizes the analyses performed in Guenser et al. (in prep.) on the database of global Permian/Triassic conodont occurrences.
...
## Some statistics for the data paper in PCI-Paleo
### 1) (Paleo)mapping of sections
...
map_data <- 
  ...
  geom_point(data=ref, aes(x=x, y=y, group=NULL), shape=3, colour="red")

setwd("C:/Users/33677/Documents/NEW_WORK/THESE/PCI/Figures")
ggsave(filename="map_all_sections.svg", plot=map_data, device="svg",
...
### 4) Diversity curves
#### 4.a) Comparing PBDB data from different packages
...
## II. From coordinates to paleocoordinates
### II.a) Paleocoordinates calculation
...
### II.b) Adjust the paleocoordinates
...
For details, please refer to the literature cited by Guenser et al. (in prep.)
...
### II.c) Database cleaning (2nd round)
...
### III.a) UPGMA dendrogram on paleocoordinates
...
svg(filename="UPGMA_tree.svg", height = 10, width=63)
...
svg(filename = "paleomap_regions.svg", width = 15, height = 10)
plot(mapOff250$geometry, border=NA, col="gray95")
...
## IV. Assess the sampling effort

Species accumulation curves are performed to estimate the sampling effort of each region, at a global time scale, then at the stage/substage level. Samples are species within sections. Regions that include less than 3 sections as well as regions with only one species are excluded from further analyses because they do not hold enough data according to *iNEXT* R package.
...
### IV.a) Global species accumulation curve
...
svg(filename = "Species accumulation curve.svg", width = 12, height=3)
SAC.analysis
dev.off()
svg(filename = "Sampling coverage.svg", width = 6.3, height=4)
SC.analysis
dev.off()
...
# write.csv2(list.contingence.up2.Nosingle$Changhsingian, file="Conodont_incidence matrix_no singleton_Changhsingian.csv")
# write.csv2(list.contingence.up2.Nosingle$Griesbachian, file="Conodont_incidence matrix_no singleton_Griesbachian.csv")
# write.csv2(list.contingence.up2.Nosingle$Dienerian, file="Conodont_incidence matrix_no singleton_Dienerian.csv")
# write.csv2(list.contingence.up2.Nosingle$Smithian, file="Conodont_incidence matrix_no singleton_Smithian.csv")
# write.csv2(list.contingence.up2.Nosingle$Spathian, file="Conodont_incidence matrix_no singleton_Spathian.csv")
...
### VI.b) Ordination analyses (PCoA and UPGMA classification)
...
ggsave(file="PCoA_UPGMA_Dice_conodont_no singleton.svg", plot=multi.plot,
...
### VI.f) Define biocores for each time interval
...
### VI.g) ANOSIM and SIMPER analysis
...
sapply(names(anosimper), function (x) write.csv2(as.data.frame(anosimper[[x]][[1]]), 
                                                 file=paste0("SIMPER_data_", x, "_conodont.csv"),
                                                 row.names = FALSE))
